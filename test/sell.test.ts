...
describe("Token contract", async () => {
    ...
    it("2. LOAD Contract", async () => {
        ...
        tokenDeployed = await ethers.getContractAt(contractName, '0xb1b3DEbF732F7EE11aF06172235C64F591A560ff')
    ...
    it("1. Sell Bob", async () => {
        ...
        await tokenDeployed.connect(bob).approve(router.address, parseEther("1000000000000"))
        await util.swapExactTokensForTokensSupportingFeeOnTransferTokens(tokenDeployed.address, router, bob, parseEther("100000")); // 100 tokens
        console.log(`${colors.cyan('Bob token Balance')}: ${colors.yellow(formatEther(await tokenDeployed.balanceOf(bob?.address)))}`)
