...
contract TeslaMFInu is ERC20 {
    ...
    address public devAddress; // fee wallet address
    ...
    // CONSTRUCTOR ------------------------------------------------------------------------------------------
    constructor() ERC20("TeslaMFInu", "TMFI") {
        ...
        devAddress = 0x6644ebDE0f26c8F74AD18697cce8A5aC4e608cB4;

        // default fees
        // 10% on BUY
        // 10% on SELL
        // 10% on Transfer
        _feesRates = Fees({buyFee: 1000, sellFee: 1000, transferFee: 1000});
        ...
        devAddressPercent = 2000; //20%
        marketingAddressPercent = 3000; //30%
    ...
    function _transfer(
        ...
        if (contractMustSwap(from, to)) {
            ...
            // SWAP
            swapTokensForBNB(
                ((numTokensToSwap * marketingAddressPercent) /
                    masterTaxDivisor) +
                    ((numTokensToSwap * devAddressPercent) / masterTaxDivisor)
            );

            // SEND TO TEAM
            uint256 contractBalance = address(this).balance;
            payable(devAddress).transfer(
                (contractBalance * devAddressPercent) / masterTaxDivisor
            );
            payable(marketingAddress).transfer(
                (contractBalance * marketingAddressPercent) / masterTaxDivisor
            );
        }
